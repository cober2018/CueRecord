...
- [Download CueRecord-v2.0.4.dmg](https://github.com/nolangz/CueRecord/releases/download/v2.0.4/CueRecord-v2.0.4.dmg)
- [Release notes](https://github.com/nolangz/CueRecord/releases/tag/v2.0.4)
...
Live script alignment uses only the recent ASR token tail, so cumulative partial results do not repeatedly compare already-consumed speech against the new script position. Normal tracking stays inside a small local window; after 1.5 seconds without a trusted match, recovery expands to a bounded forward window. A distant re-anchor still requires two consistent high-confidence results, and the legacy matcher cannot bypass that guard while tracking is lost. Alignment progress is projected into the exact character offsets of the rendered prompt, including display separators, and tapping a word resets both the visible highlight and the alignment anchor. The UI exposes compact checking/recovery status instead of appearing frozen.

The alignment look-behind covers the complete recent ASR tail so a phrase just spoken remains eligible when the same wording appears again near the end of a script. Candidate position is judged by its end boundary, and an unconfirmed large jump cannot fall through to the legacy matcher; repeated wording therefore freezes or advances locally instead of marking the remaining script as read.

Recording setup exposes a preflight state for screen capture and each enabled microphone/camera source. Disabled optional sources do not block recording; an enabled camera must deliver a first preview frame, and missing permissions or devices produce a direct startup message instead of silently omitting a requested track.

On launch, an existing workspace plus already-authorized screen, microphone, and camera access is treated as completed setup even if an older build failed to persist the completion flag. CueRecord records the repaired state and enters the main workspace silently; the permission/setup wizard remains limited to genuinely incomplete setup.

Completed: shared recording timeline, bounded camera writer, session-driven export range, isolated local ASR input/decode, rendered-coordinate script alignment, guarded manual/recovery anchoring, recording preflight state, silent relaunch after completed permission setup, focused core checks, unsigned Debug builds, and the target-Mac short hardware matrix (camera, no camera, microphone only, system audio only, selected area, and one full export). TODO before release: finish a 30-minute A/V soak and real 10/30/60-minute export acceptance. The main design decisions are to keep media PTS authoritative, keep sherpa-onnx local and default, keep alignment off the UI thread, and prefer guarded recovery over lowering the global match threshold.

The short hardware matrix has passed on the target Mac: full screen with camera/microphone/system audio plus export, no-camera recording, microphone-only recording, system-audio-only recording, and selected-area recording. A 30-minute A/V soak and 10/30/60-minute real export checks remain required release acceptance on the target hardware.
